...
if __name__ == "__main__":
    ...
    if article_limit:
        try:
            ...
            print(
                f'Env var set for article_limit -> used instead of config.json value = {config["ner"]["article_limit"]}'
            )
            start, end = map(int, article_limit.split(":"))
            config["ner"]["article_limit"] = [
                start,
                end,
            ]  # Replace the JSON value with the environment variable
            print(f'article_limit set to {config["ner"]["article_limit"]}')
        except ValueError:
            print(
                f"Invalid ARTICLE_LIMIT format: {article_limit}. Expected format: 'start:end'."
            )
    else:
