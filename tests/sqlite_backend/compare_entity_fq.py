...
def compare_total_count(tsv_file, csv_file, output_file) -> None:
    ...
    merged_data = pd.merge(
        tsv_data,
        csv_data,
        left_on=tsv_data.columns[0],
        right_on="entity_text",
        how="outer",
        suffixes=("_json", "_database"),
        indicator=True,
    )

    # Calculate the total_count difference
    merged_data["total_count_diff"] = (
        merged_data["total_count_json"] - merged_data["total_count_database"]
    )

    # Filter the differences where total_count values differ
    diff_data = merged_data[
        (merged_data["_merge"] != "both") | (merged_data["total_count_diff"] != 0)
    ]

    # Select relevant columns for output
    output_data = diff_data[
        ["entity_text", "total_count_json", "total_count_database", "total_count_diff"]
    ]
