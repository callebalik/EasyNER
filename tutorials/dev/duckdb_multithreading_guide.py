"""Demo simultaneously insert into and read from a DuckDB database across multiple Python threads.

 This could be useful in scenarios where new data is flowing in and
 an analysis should be periodically re-run.
 Note that this is all within a single Python process.
(see the FAQ for details on DuckDB concurrency).
...
# Each thread must use the .cursor() method to create a thread-local connection
# to the same DuckDB file based on the original connection.
# This approach also works with in-memory DuckDB databases.


def write_from_thread(duckdb_con: duckdb.DuckDBPyConnection) -> None:
...
def read_from_thread(duckdb_con: duckdb.DuckDBPyConnection) -> None:
    """Create a DuckDB connection specifically for this thread."""
    local_con = duckdb_con.cursor()
...
# We define how many writers and readers to use,
# and define a list to track all of the threads that will be created.
# Then, create first writer and then reader threads.
# Next, shuffle them so that they will be kicked off in a random order
# to simulate simultaneous writers and readers.
...
for i in range(write_thread_count):
    threads.append(  # noqa: PERF401
        Thread(
            target=write_from_thread,
            args=(duckdb_con,),
...
# Now, kick off all threads to run in parallel,
# then wait for all of them to finish before printing out the results.
# Note that the timestamps of readers and writers are interspersed as
# expected due to the randomization.
