...
def get_running_job_details(job_id):
    ...
    squeue_result = subprocess.run(['squeue', '--job', job_id, '--format=%M', '--noheader'],
...
def get_completed_job_details(job_id):
    ...
    sacct_result = subprocess.run(['sacct', '--jobs', f'{job_id}.batch', '--format=JobID,Elapsed,CPUTime,State,ExitCode', '--parsable2'],
                                  stdout=subprocess.PIPE, universal_newlines=True)
...
def monitor_jobs(job_metadata, output_file, log_file):
    ...
    monitoring_results = []
    table = PrettyTable()
    table.field_names = ["Batch", "Job ID", "Job Name", "Status", "Elapsed Time", "CPU Usage", "Exit Code"]
    ...
    for batch, job_info in job_metadata.items():
        ...
        if job_id:
            ...
            squeue_result = subprocess.run(['squeue', '--job', job_id], stdout=subprocess.PIPE, universal_newlines=True)
            ...
            table.add_row([
                batch, job_id, job_name, result.get('status', 'UNKNOWN'),
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Monitor SLURM jobs using job_metadata.json and export results to a file")
    parser.add_argument('--metadata-file', type=str, required=True, help="Path to the job metadata file (job_metadata.json)")
    parser.add_argument('--output-file', type=str, required=True, help="Path to the output file for monitoring results (JSON)")
    parser.add_argument('--log-file', type=str, required=True, help="Path to the log file for formatted results (Table)")
