...
from ..statistics.sankey_diagram import CooccurenceSankey
...
import logging
import threading
from .errors import SankeyVisualizationError
from ..data_model.schema import *
from contextlib import contextmanager

logger = logging.getLogger("EasyNerDB")

def register_disease_phenomena_routes(app, get_db_simple_connection, visualization_manager: VisualizationManager):
    ...
    def disease_phenomena():
        ...
        """
        try:
            logger.info("Accessing disease-phenomena page", extra={
            ...
            logger.debug("Received search parameters", extra={
            ...
            logger.debug("Filter parameters", extra={
                'min_npmi': min_npmi,
                'max_npmi': max_npmi,
                'min_pmi': min_pmi,
                'max_pmi': max_pmi,
            ...
            if min_pmi != DEFAULT_MIN_PMI: filters.append(f"minp_{min_pmi}")
            if max_pmi is not None: filters.append(f"maxp_{max_pmi}")
            if min_npmi != DEFAULT_MIN_NPMI: filters.append(f"minn_{min_npmi}")
            if max_npmi is not None: filters.append(f"maxn_{max_npmi}")
            if min_doc_fq != DEFAULT_MIN_DOC_FQ: filters.append(f"minf_{min_doc_fq}")
            if max_doc_fq is not None: filters.append(f"maxf_{max_doc_fq}")
            if min_uniq_docs is not None: filters.append(f"minu_{min_uniq_docs}")
            if max_uniq_docs is not None: filters.append(f"maxu_{max_uniq_docs}")

            cache_key = 'sankey_' + ('base' if not filters else '_'.join(filters))
            logger.debug("Generated cache key", extra={'cache_key': cache_key, 'filters': filters})
            ...
            try:
                def generate_sankey_visualization(_):
                    ...
                    logger.debug(f"Starting sankey diagram generation in thread {thread_id}")
                    ...
                    try:
                        ...
                        if max_pmi is not None:
                            filter_dict['max_pmi'] = float(max_pmi) if max_pmi != '' else None
                        if min_npmi is not None:
                            filter_dict['min_npmi'] = float(min_npmi) if min_npmi != '' else None
                        if max_npmi is not None:
                            filter_dict['max_npmi'] = float(max_npmi) if max_npmi != '' else None
                        if min_doc_fq is not None:
                            # Use lowercase column name that matches the database schema
                            filter_dict['min_fq_doc_level'] = float(min_doc_fq) if min_doc_fq != '' else None
                        if max_doc_fq is not None:
                            # Use lowercase column name that matches the database schema
                            filter_dict['max_fq_doc_level'] = float(max_doc_fq) if max_doc_fq != '' else None
                        if min_uniq_docs is not None:
                            filter_dict['min_uniq_docs'] = float(min_uniq_docs) if min_uniq_docs != '' else None
                        if max_uniq_docs is not None:
                            filter_dict['max_uniq_docs'] = float(max_uniq_docs) if max_uniq_docs != '' else None
                        logger.debug(f"Filter dictionary for data exchange: {filter_dict}")

                        logger.debug(f"Fetching cooccurrence data in thread {thread_id} with filters: {filter_dict}")
                        ...
                        if not cooccurrences:
                            logger.warning(f"No cooccurrences found with current filters in thread {thread_id}")
                            return """
                        ...
                        logger.info(f"Found {len(cooccurrences)} cooccurrences in thread {thread_id}")
                        ...
                        logger.info(f"Sankey diagram generation complete in thread {thread_id}")
                        return result
                    except Exception as e:
                        logger.error(f"Error in thread {thread_id}: {str(e)}", exc_info=True)
                        return f"""
                ...
                logger.error("Error generating sankey diagram", exc_info=True, extra={
                    'cache_key': cache_key,
                    'error': str(e)
                })
                diagram_html = f"""
            ...
            logger.debug("Rendering template with parameters", extra={
                'defaults': defaults,
                'has_diagram': diagram_html is not None
            })
            ...
            logger.error("Error rendering disease phenomena page", exc_info=True, extra={
                'error': str(e),
