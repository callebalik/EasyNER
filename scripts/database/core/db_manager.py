...
class DatabaseManager:
    ...
    def __init__(self, db_path: str = None, config_path: str = "config.json", logger: BaseLogger = None):
        """
        ...
        self.db_path = db_path or self.config.get("db_path") # Use provided path or from config

        if logger:
            self.logger = logger
        else:
            self.logger = BaseLogger("DatabaseManagerLogger") # Default logger if none provided
            self._setup_default_logging() # Setup file logging for default logger

        # Development Mode Handling
        if self.config.get("develop"):
            self.logger.info("Setting up in development mode, ignoring database path provided if any.")
            pwd = os.path.dirname(os.path.abspath(__file__))
            self.db_path = os.path.join(pwd, "development.db")

        if not self.db_path:
            self.logger.warning("No database path provided, defaulting to path from config.")
            self.db_path = self.config.get("db_path")

        # Resolve db_path to be absolute
        if not os.path.isabs(self.db_path):
            project_root = os.path.dirname(os.path.abspath(config_path)) # Config path as project root
            self.db_path = os.path.join(project_root, self.db_path)

        # Database Creation if not exists
        if not os.path.exists(self.db_path):
            self.logger.info(f"Database {self.db_path} does not exist. Creating database...")
            schema_path = self.config.get("schema_path")
            if os.getenv("DB_RUN", False) == True or not sys.stdin.isatty(): # Non-interactive env
                self.logger.info("Non-interactive environment, using default schema path from config.")
    ...
    def _load_config(self, config_path):
        ...
        config_dir = os.path.dirname(os.path.abspath(__file__)) # Assuming config near script
        ...
        try:
            with open(full_config_path, "r") as f:
                ...
                if not config_data.get("develop") and not config_data.get("db_path"):
                    raise ValueError("Database configuration must contain 'db_path' unless in development mode.")
                if not config_data.get("schema_path"):
                    raise ValueError("Database configuration must contain 'schema_path'.")
                return config_data
        except (FileNotFoundError, json.JSONDecodeError, ValueError) as e:
            raise ValueError(f"Error loading database configuration from {config_path}: {e}")
    ...
    def _setup_default_logging(self):
        ...
        self.logger.logger.addHandler(file_handler) # Access underlying logger from BaseLogger
        self.logger.logger.addHandler(console_handler)
        self.logger.logger.addHandler(error_file_handler)

        self.logger.info(f"Default logging initialized. Log file: {log_file}, Error log file: {error_log_file}")
    ...
    def close_connection(self):
        ...
        if hasattr(self, '_connection') and self._connection: # Check if connection exists and is not None
            try:
    ...
    def optimize_db_performance_parameters(self):
        ...
        self.execute("PRAGMA journal_size_limit = 6144000")
        self.execute("PRAGMA temp_store = MEMORY")
        self.logger.info("Database performance parameters optimized for balanced performance.")
    ...
    def optimize_db_performance_for_write(self):
        ...
        self.execute("PRAGMA journal_size_limit = 6144000")
        self.execute("PRAGMA temp_store = MEMORY")
        self.logger.info("Database performance parameters optimized for write operations.")

    def optimize_db_performance_for_read(self):
        """Optimize database for read-heavy operations."""
        self.execute("PRAGMA journal_mode = MEMORY") # In-memory journaling, fastest reads, less durable
        self.execute("PRAGMA synchronous = OFF") # Less safety, faster reads
        self.execute("PRAGMA journal_size_limit = 6144000")
        self.execute("PRAGMA temp_store = MEMORY")
        self.logger.info("Database performance parameters optimized for read operations.")
    ...
    def align_with_schema(self, schema_path=None):
        ...
        if not os.path.isabs(schema_path):
            config_dir = os.path.dirname(os.path.abspath(__file__)) # Config path as project root
        ...
        schema_tables = [row[0] for row in schema_cursor.execute("SELECT name FROM sqlite_master WHERE type='table';").fetchall()]
        ...
        for table in schema_tables:
            ...
            schema_columns = {col[1]: col for col in schema_cursor.execute(f"PRAGMA table_info('{table}');").fetchall()}
            db_columns = {col[1]: col for col in self.execute_query(f"PRAGMA table_info('{table}');")}

            if table not in self.get_table_names():
                cols_def = ', '.join([f"{col[1]} {col[2]}{' NOT NULL' if col[3] else ''}{' DEFAULT '+str(col[4]) if col[4] is not None else ''}{' PRIMARY KEY' if col[5] else ''}" for col in schema_columns.values()])
                ...
                for col_name, col_info in schema_columns.items():
                    if col_name not in db_columns:
                        add_stmt = f"ALTER TABLE {table} ADD COLUMN {col_name} {col_info[2]}"
    ...
    def clear_connections(self):
        ...
        try:
            ...
            self.logger.error(f"Error committing pending transactions during clear connections: {e}")

        try:
            self.execute_query("PRAGMA wal_checkpoint(FULL);", commit=True)
        except Exception as e:
            self.logger.error(f"Error executing WAL checkpoint during clear connections: {e}")
        finally:
            self.close_connection()
    ...
    def create_db(cls, db_path, schema_path):
        ...
        try:
            ...
            print(f"Schema file not found: {schema_path}") # Use print here as logger might not be setup yet
            return
        ...
        try:
            ...
            conn.commit()
            print(f"Database created at {db_path} using schema from {schema_path}") # Use print here
        except sqlite3.Error as e:
