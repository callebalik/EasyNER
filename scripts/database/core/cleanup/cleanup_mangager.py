...
class CleanupManager:
    ...
    def __init__(self, db_manager: DatabaseManager, logger: BaseLogger):
        ...
        self.logger = logger
        self._scheduled_tasks = [] # List of (task_id, schedule_str, task_callable, args, kwargs)
        self._scheduler_thread = None
        self._is_running = False

    def register_task(self, task_id, schedule_str, task_callable, *args, **kwargs):
        ...
        self.logger.info(f"Cleanup task '{task_id}' registered with schedule '{schedule_str}'.")
    ...
    def schedule_tasks(self):
        ...
        for task_config in self._scheduled_tasks:
            ...
            try:
                ...
                if schedule_type in schedule_fns:
                    schedule_job = schedule_fns[schedule_type](self._run_cleanup_task, task_id=task_id, task_callable=task_callable, args=args, kwargs=kwargs)
                    if schedule_type == "daily" and schedule_args:
                        ...
                        schedule_job.on(schedule_args[0]) # Set weekday for weekly tasks

                else:
                    self.logger.error(f"Invalid schedule type '{schedule_type}' for task '{task_id}'.")
                    continue
        ...
        self._scheduler_thread.daemon = True # Allow main thread to exit without waiting
    ...
    def _run_cleanup_task(self, task_id, task_callable, args, kwargs):
        ...
        try:
            task_callable(self.db_manager, self.logger, *args, **kwargs) # Pass db_manager and logger
            end_time = time.time()
            duration = end_time - start_time
            self.logger.info(f"Cleanup task '{task_id}' completed in {duration:.4f} seconds.")
        except Exception as e:
    ...
    def remove_task(self, task_id):
        ...
        self._scheduled_tasks = [task for task in self._scheduled_tasks if task['task_id'] != task_id]
    ...
    def stop_scheduler(self):
        ...
        if self._is_running:
            ...
            if self._scheduler_thread and self._scheduler_thread.is_alive():
                self._scheduler_thread.join(timeout=5) # Wait for thread to finish gracefully
