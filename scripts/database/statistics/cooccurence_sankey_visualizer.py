...
from scripts.database.statistics.color_scheme import (
    NODE_COLOR_DIS_ONLY, NODE_COLOR_PNM_ONLY,
    NODE_COLOR_BOTH_DIS_PNM, NODE_COLOR_DEFAULT,
...
class CooccurrenceSankeyVisualizer:
    ...
    def __init__(self, source_category_name="Disease", target_category_name="Phenomenon",
                 source_base_color=None, target_base_color=None,
                 link_opacity=0.6):
        """
    ...
    def create_node_mapping(self, unique_sources, unique_targets):
        ...
        target_indices = {t: i + len(unique_sources) for i, t in enumerate(unique_targets)}
    ...
    def _generate_color_gradient(self, base_color: str, count: int, randomize: bool = True) -> List[str]:
        """
        ...
        if rgb_color.startswith('rgb('):
            ...
            match = re.match(r'rgba\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*,\s*([0-9.]+)\s*\)', rgb_color)
        ...
        for i in range(count):
            ...
            # Create rgba string
            colors.append(f"rgba({int(new_r * 255)}, {int(new_g * 255)}, {int(new_b * 255)}, 0.8)")
    ...
    def _mix_colors(self, source_color: str, target_color: str, mix_ratio: float = 0.5) -> str:
        """
        ...
        """
        # Parse source color
        source_match = re.match(r'rgba\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*,\s*([0-9.]+)\s*\)', source_color)
        if not source_match:
            source_match = re.match(r'rgb\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*\)', source_color)
            if source_match:
                sr, sg, sb = map(int, [source_match.group(1), source_match.group(2), source_match.group(3)])
                sa = 1.0
            else:
                sr, sg, sb, sa = 128, 128, 128, self.link_opacity  # Default gray
        else:
            sr, sg, sb, sa = map(float, [source_match.group(1), source_match.group(2),
                                         source_match.group(3), source_match.group(4)])
            sr, sg, sb = int(sr), int(sg), int(sb)

        # Parse target color
        target_match = re.match(r'rgba\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*,\s*([0-9.]+)\s*\)', target_color)
        if not target_match:
            target_match = re.match(r'rgb\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*\)', target_color)
            if target_match:
                tr, tg, tb = map(int, [target_match.group(1), target_match.group(2), target_match.group(3)])
                ta = 1.0
            else:
                tr, tg, tb, ta = 128, 128, 128, self.link_opacity  # Default gray
        else:
            tr, tg, tb, ta = map(float, [target_match.group(1), target_match.group(2),
                                         target_match.group(3), target_match.group(4)])
    ...
    def create_diagram(self, source_entities, target_entities, links, title=None):
        ...
        """
        # Process data
        unique_sources, unique_targets = self.extract_unique_entities(source_entities, target_entities)
        node_labels, source_indices, target_indices = self.create_node_mapping(unique_sources, unique_targets)
        link_sources, link_targets, link_values = self.transform_links(links, source_indices, target_indices)

        # Generate color gradients for source and target nodes
        source_colors = self._generate_color_gradient(self.source_base_color, len(unique_sources))
        target_colors = self._generate_color_gradient(self.target_base_color, len(unique_targets))
        ...
        # Update layout
        diagram_title = title if title else f"{self.source_category_name}-{self.target_category_name} Co-occurrence Network"
    ...
    def save(self, filepath, format=None):
        ...
        if self.figure is not None:
            ...
            raise ValueError("The figure has not been created. Please create the diagram before saving.")
        return self

    def create_diagram_from_cooccurrences(self, cooccurrences: list[Cooccurrence], title=None):
        """
