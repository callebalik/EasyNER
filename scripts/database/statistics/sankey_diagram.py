...
class CooccurenceSankey:
        ...
        try:
            ...
            for entity in labels:
                if entity in e1_data:
                    customdata.append(
                        [e1_data[entity][FQ_DOCUMENT_LEVEL], e1_data[entity][UNIQ_DOCS]]
                    )
                else:
                    customdata.append(
                        [e2_data[entity][FQ_DOCUMENT_LEVEL], e2_data[entity][UNIQ_DOCS]]
                    )

            # Create Sankey diagram
            # Use the average of node thicknesses since Plotly expects a single number, not a list
            avg_thickness = (
                sum(node_thicknesses) / len(node_thicknesses)
                if node_thicknesses
                else 20
            )
            ...
            self.logger.error(
                f"Error generating Sankey diagram: {str(e)}", exc_info=True
            )
            return f"""
    ...
    def export_as_html(self, cooccurrences: list[Cooccurrence], filename: str):
        ...
        try:
            ...
            self.logger.error(
                f"Error exporting Sankey diagram: {str(e)}", exc_info=True
            )

    def export_as_image(
        self, cooccurrences: list[Cooccurrence], filename: str = "sankey_diagram.png"
    ):
    ...
    def _process_data(self, cooccurrences: list[Cooccurrence]):
        ...
        for cooc in cooccurrences:
            ...
            e1_fq = (
                cooc.e1.fq if cooc.e1 and hasattr(cooc.e1, "fq") else cooc.fq_doc_level
            )
            e1_uniq_docs = (
                cooc.e1.uniq_docs
                if cooc.e1 and hasattr(cooc.e1, "uniq_docs")
                else cooc.uniq_docs
            )
            e2_fq = (
                cooc.e2.fq if cooc.e2 and hasattr(cooc.e2, "fq") else cooc.fq_doc_level
            )
            e2_uniq_docs = (
                cooc.e2.uniq_docs
                if cooc.e2 and hasattr(cooc.e2, "uniq_docs")
                else cooc.uniq_docs
            )

            e1_data[e1_txt] = {FQ_DOCUMENT_LEVEL: e1_fq, UNIQ_DOCS: e1_uniq_docs}
            e2_data[e2_txt] = {FQ_DOCUMENT_LEVEL: e2_fq, UNIQ_DOCS: e2_uniq_docs}
    ...
    def _create_node_colors(self, processed_data: dict):
        ...
        def _generate_colors(
            nodes, coloring_parameter: str, hue: int, saturation_range: tuple
        ):
            """Generate colors for nodes based on a parameter."""
            colors = []
            max_count = max(
                (data[coloring_parameter] for data in nodes.values()), default=1
            )
            ...
            for node in nodes:
                ...
                saturation = min(
                    saturation_range[1]
                    + (np.log1p(count) / np.log1p(max_count)) * saturation_range[0],
                    100,
                )
                colors.append(f"hsl({hue}, {saturation}%, 70%)")
            return colors

        disease_colors = _generate_colors(
            disease_data, UNIQ_DOCS, hue=210, saturation_range=(50, 70)
        )
        phenomenon_colors = _generate_colors(
            phenomenon_data, FQ_DOCUMENT_LEVEL, hue=210, saturation_range=(50, 70)
        )
    ...
    def _create_link_data(self, processed_data):
        ...
        max_fq_doc_level = max(
            (getattr(cooc, "fq_doc_level", 1) or 1 for cooc in cooccurrences), default=1
        )
        max_uniq_docs = max(
            (getattr(cooc, "uniq_docs", 1) or 1 for cooc in cooccurrences), default=1
        )
    ...
    def _calculate_node_thicknesses(self, processed_data):
        ...
        max_disease_fq = max(
            (data[FQ_DOCUMENT_LEVEL] for data in e1_data.values()), default=1
        )
        max_phenomenon_fq = max(
            (data[FQ_DOCUMENT_LEVEL] for data in e2_data.values()), default=1
        )
