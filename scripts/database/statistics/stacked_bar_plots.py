...
class EntityDistributionPlotter:
    ...
    def __init__(
        ...
        self.highlight_colors = {
            "PNM": PNM_HIGHLIGHT_COLORS[
                0.2
            ],  # Using pre-computed highlight color with 0.2 alpha
            "DIS": DIS_HIGHLIGHT_COLORS[
                0.2
            ],  # Using pre-computed highlight color with 0.2 alpha
        }
        ...
        if self.layout == "vertical":
            self.fig, (self.ax_percentages, self.ax_counts) = plt.subplots(
                2, 1, figsize=self.figsize, constrained_layout=False
            )
        else:
            self.fig, (self.ax_percentages, self.ax_counts) = plt.subplots(
                1, 2, figsize=self.figsize, constrained_layout=False
            )
    ...
    def setup_plot_parameters(self):
        ...
        if self.layout == "vertical":
            ...
            plt.tight_layout(
                rect=[0, 0.03, 1, 0.90]
            )  # Make room for shared legend at top
            plt.subplots_adjust(
    ...
    def create_percentage_subplot(self):
        ...
        self.ax_percentages.set_yticklabels(
            self.classes, fontsize=self.MEDIUM_FONT_SIZE
        )
        self.ax_percentages.set_xlabel(
            "Percentage of named entity (%) - Log Scale",
            fontsize=self.SMALL_FONT_SIZE,
            fontweight="bold",
        )
        self.ax_percentages.set_ylabel(
            "Named Entity Class", fontsize=self.MEDIUM_FONT_SIZE, fontweight="bold"
        )
        self.ax_percentages.set_title(
            "A: Distribution by Percentage",
            fontsize=self.LARGE_FONT_SIZE,
            fontweight="bold",
        )
    ...
    def _add_text_labels_to_percentage_bars(self):
        ...
        for i, (ambig, mislab, valid) in enumerate(
            zip(self.ambig_percentages, self.mislab_percentages, self.valid_percentages)
        ):
            # Ambiguous segment
            if ambig > 0.01:
                log_mid_point = (
                    np.log10(mislab + self.EPSILON + ambig / 2) * 0.1
                    if ambig > 0
                    else 0
                )
                self.ax_percentages.text(
                    10**log_mid_point,
                    i,
                    f"{ambig:.2f}%",
                    ha="center",
            ...
            if mislab > 0.001:
                ...
                self.ax_percentages.text(
                    ...
                    va="center",
                    fontweight="bold",
                    color="white" if mislab > 5 else "black",
                    fontsize=self.SMALL_FONT_SIZE,
                )
            ...
            if valid > 0.5:  # Only show if enough space
                ...
                if mid_point_orig > 70:
                    ...
                    self.ax_percentages.text(
                        ...
                        va="center",
                        fontweight="bold",
                        color="black" if valid > 20 else "white",
                        fontsize=self.SMALL_FONT_SIZE,
                    )
    ...
    def create_counts_subplot(self):
        ...
        self.ax_counts.set_xlabel(
            "Entity Count - Log scale", fontsize=self.SMALL_FONT_SIZE, fontweight="bold"
        )
        self.ax_counts.set_ylabel(
            "Named Entity Class", fontsize=self.SMALL_FONT_SIZE, fontweight="bold"
        )
        self.ax_counts.set_title(
            "B: Distribution by Count", fontsize=self.LARGE_FONT_SIZE, fontweight="bold"
        )
        ...
        self.ax_counts.set_xticklabels(
            [self._format_count(x) for x in log_count_ticks],
            fontsize=self.SMALL_FONT_SIZE,
        )
    ...
    def _add_text_labels_to_count_bars(self):
        ...
        for i, (ambig, mislab, valid) in enumerate(
            zip(self.ambig_counts, self.mislab_counts, self.valid_counts)
        ):
            # Ambiguous segment
            if ambig > 10:
                log_mid_point = (
                    np.log10(mislab + self.EPSILON + ambig / 2) * 0.7
                    if ambig > 0
                    else 0
                )
                self.ax_counts.text(
            ...
            if mislab > 10:
                ...
                self.ax_counts.text(
                    10**log_mid_point,
                    i,
                    self._format_count(mislab),
                    ha="center",
                    va="center",
                    fontweight="bold",
                    color="white" if mislab > 1000 else "black",
                    fontsize=self.SMALL_FONT_SIZE,
                )
