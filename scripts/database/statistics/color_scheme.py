...
def convert_color_to_rgb(color_str: str) -> str:
    ...
    if color_str.startswith('hsl'):
        ...
        match = re.match(r'rgba\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*,\s*([0-9.]+)\s*\)', color_str)
        if match:
            r = match.group(1)
            g = match.group(2)
            b = match.group(3)
...
def get_matplotlib_color(color_str: str) -> Union[str, Tuple[float, float, float, float]]:
    """
...
LINK_COLOR_WITH_ENTITIES_TO_BOTH = "rgba(140, 150, 210, 0.4)" # Matching the mixed color
...
# Link colors (RGB)
LINK_COLOR_TOTAL_TO_WITH_ENTITIES_RGB = get_matplotlib_color(LINK_COLOR_TOTAL_TO_WITH_ENTITIES)
LINK_COLOR_TOTAL_TO_WITHOUT_ENTITIES_RGB = get_matplotlib_color(LINK_COLOR_TOTAL_TO_WITHOUT_ENTITIES)
LINK_COLOR_WITH_ENTITIES_TO_DIS_ONLY_RGB = get_matplotlib_color(LINK_COLOR_WITH_ENTITIES_TO_DIS_ONLY)
LINK_COLOR_WITH_ENTITIES_TO_BOTH_RGB = get_matplotlib_color(LINK_COLOR_WITH_ENTITIES_TO_BOTH)
LINK_COLOR_WITH_ENTITIES_TO_PNM_ONLY_RGB = get_matplotlib_color(LINK_COLOR_WITH_ENTITIES_TO_PNM_ONLY)
