...
class TableFormatter:
    ...
    @staticmethod
    def format_table(data: Dict[str, Union[str, int, float, bool]],
                     title: Optional[str] = None,
                     column_separator: str = '│',
                     edge_char: str = '│',
                     header_separator: str = '─',
                     min_width: int = 50) -> str:
        """
        ...
        separator = header_separator * (key_width + val_width + 5)  # +5 for separators and spaces
        ...
        if title:
            ...
            lines.append(f"{edge_char} {title:^{key_width + val_width + 3}} {edge_char}")
    ...
    def parse_and_format(log_text: str, title: Optional[str] = None) -> str:
        ...
        if not data:
            patterns = [
                (r'(\w+(?:\s+\w+)?)\s*[:=]\s*(.*)', lambda m: (m.group(1), m.group(2))),  # key: value or key = value
                (r'(\w+(?:\s+\w+)?)\s+((?:\d+(?:\.\d+)?)|(?:True|False|None))\s*\.?', lambda m: (m.group(1), m.group(2))),  # key 123 or key True
            ]
    ...
    def from_reader_writer_log(cls, log_text: str) -> str:
        ...
        reader_threads_match = re.search(r'Reader threads: Started\s+(\d+)/(\d+)', log_text)
        if reader_threads_match:
            data['Reader threads running'] = f"{reader_threads_match.group(1)}/{reader_threads_match.group(2)}"
