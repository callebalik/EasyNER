...
class JsonDiagnostic:
    ...
    def __init__(self, json_dir=None):
        ...
        self.error_samples = {error_type: [] for error_type in self.error_types}
        ...
        try:
            ...
            with open(file_path, "r", encoding="utf-8") as f:
                try:
                    ...
                    result["error_details"] = f"File has encoding issues: {str(ue)}"
                    ...
                    if detailed and result["line_number"] and result["position"]:
                        with open(
    ...
    def _analyze_structure(self, data, result):
        ...
        if isinstance(data, dict):
            ...
            if not has_title:
                ...
                result["error_details"] = "No 'title' field found in document entries"
    ...
    def repair_file(self, file_path, output_path=None, repair_strategy="auto"):
        ...
        if error_type == "empty":
            ...
            return True, "Empty file replaced with minimal valid JSON object.", "empty"

        if error_type == "not_found":
            return False, f"Cannot repair: {analysis['error_details']}", "not_found"
        ...
        try:
            ...
            try:
                ...
                return False, f"Repair attempt failed validation: {str(e)}", error_type
...
Examples:
  ...
  python json_diagnostics.py repair data/ --output-dir fixed_data/
    ...
    analyze_parser.add_argument(
        "--detailed", action="store_true", help="Show detailed error information"
    )
    analyze_parser.add_argument(
    ...
    if args.command == "analyze":
        if len(diagnostic.json_files) == 1:
            ...
            if result.get("line_number"):
                print(f"Line {result['line_number']}, Position {result['position']}")
