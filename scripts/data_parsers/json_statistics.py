...
class ArticleCounter:
    ...
    def run_threaded_counter(self):
        ...
        try:
            ...
            self.results = {
                ...
                "errors": len(errors),
                "error_details": errors[:5] if errors else [],  # Include first 5 errors
            ...
            if len(errors) > 0 and len(errors) == len(self.processed_files):
                ...
                if errors:
                    ...
                    print(f"{err_msg[:500]}..." if len(err_msg) > 500 else err_msg)

                    # Check for common JSON structure issues
                    if self.processed_files:
                        sample_file = self.processed_files[0]
                        print(
                            f"\nChecking structure of first file: {os.path.basename(sample_file)}"
                        )
                        try:
                            with open(sample_file, "r", encoding="utf-8") as f:
                                ...
                                if first_char == "{":
                                    ...
                                    print(
                                        "File starts with an array, but code expects a dictionary/object."
                                    )
                                else:
                                    print(
                                        f"File doesn't start with a valid JSON structure (starts with '{first_char}')."
                                    )
                        except Exception as e:
    ...
    def run_multiprocessing_counter(self):
        ...
        print(
            f"Processing with {self.num_processes} processes ({self.num_processes/available_cores:.1f}x CPU utilization)..."
        )
        ...
        try:
                ...
                def update_progress(result):
                    ...
                    if "error" not in result:
                        progress_bar.set_postfix(
                            articles=f"{total_articles:,}", errors=f"{len(errors)}"
                        )
                    return result
            ...
            print(f"\nProcessing complete:")
            print(
                f"- Speed: {files_per_second:.2f} files/sec, {articles_per_second:.2f} articles/sec"
            )
            print(
                f"- Files: {len(processed_files):,} of {len(self.json_files):,} ({100*len(processed_files)/len(self.json_files):.1f}%)"
            )
            print(
                f"- Articles: {total_articles:,} ({len(all_combinations):,} unique, {self.results['duplicate_rate']:.2f}% duplicates)"
            )
    ...
    def print_summary(self):
        ...
        if "errors" in self.results and self.results["errors"] > 0:
            print(f"Encountered {self.results['errors']} errors during processing")
    ...
    def generate_report(self, output_path=None):
        ...
        report_data = {
            "summary": {
                ...
                "total_files": self.results.get("total_files", len(self.json_files)),
            ...
            report_data["file_statistics"] = [
                {
                    ...
                    "processing_time_seconds": round(stat["processing_time"], 3),
                }
    ...
    def generate_duplicate_report(self, output_path=None):
        ...
        try:
            ...
            if duplicates:
                ...
                print(f"Duplicate report saved to: {output_path} (No duplicates found)")
            return True
        except Exception as e:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "json_dir", help="Directory containing the JSON files to analyze"
    )
    parser.add_argument(
        "--threads",
        type=int,
    ...
    parser.add_argument(
        "--duplicate_report",
        metavar="FILEPATH",
        help="Generate a detailed JSON duplicate report (default: duplicate_report_<timestamp>.json)",
    )
