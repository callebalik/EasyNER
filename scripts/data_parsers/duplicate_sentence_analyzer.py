...
class DuplicateSentenceAnalyzer:
    ...
    def load_duplicate_report(self):
        ...
        try:
            ...
            if self.verbose:
                print(f"[DEBUG] Found {duplicate_count} duplicate sets in the report")
    ...
    def calculate_sentence_similarity(self, sentence1, sentence2):
        ...
        # Only use the more expensive SequenceMatcher if sentences aren't identical
    ...
    def analyze_duplicate_pair(self, duplicate_info, file_paths):
        ...
        try:
            ...
            for file_path in files:
                ...
                try:
                    with open(
                        ...
                        data = json.load(f)
                        # Check if the article with this PMID exists in this file
                        if pmid in data:
            ...
            for i in range(len(file_contents)):
                for j in range(i + 1, len(file_contents)):  # Quadratic complexity
                    file1_path, article1_data = file_contents[i]
                    file2_path, article2_data = file_contents[j]

                    comparison_result = self.compare_article_sentences(  # This is computationally expensive
            ...
            return {
                ...
                "status": "error",
                "error": str(e),
                "files": duplicate_info["files"] if "files" in duplicate_info else [],
            }
    ...
    def run_analysis(self):
        ...
        try:
            ...
            print(
                f"Found {len(file_to_duplicates)} unique files referenced by duplicates"
            )

            print("Step 2/3: Creating optimized processing batches...")
            # Create batches of duplicates that share files
            duplicate_batches = self._create_duplicate_batches(file_to_duplicates)
            ...
            print(
                f"Batch size statistics: min={min(batch_sizes)}, max={max(batch_sizes)}, avg={sum(batch_sizes)/len(batch_sizes):.1f}"
            )
                ...
                for i, future in enumerate(
                    ...
                    try:
                        ...
                        if (i + 1) % 10 == 0 or i == len(futures) - 1:
                            print(
                                f"Processed {i+1}/{len(futures)} batches ({(i+1)/len(futures)*100:.1f}%)"
                            )

                    except TimeoutError:
                        print(
                            f"WARNING: Batch timed out after {TIMEOUT_PER_BATCH} seconds. Skipping."
                        )
                    except Exception as e:
    ...
    def _create_duplicate_batches(self, file_to_duplicates):
        ...
        if self.verbose:
            print(f"Building connection graph for {len(duplicate_to_id)} duplicates...")
        ...
        for file_path, dups in tqdm(
            ...
            if len(dups) > 1000:
                if self.verbose:
                    print(f"Skipping file with {len(dups)} duplicates: {file_path}")
                continue
        ...
        if self.verbose:
            print(f"Finding connected components for {len(connections)} nodes...")
    ...
    def _merge_small_batches(self, batches, target_batch_count=None):
        ...
        # Create a heap of (size, index) tuples - don't include the batch objects
        ...
        while len(heap) + len(result) - len(merged) > target_batch_count:
            ...
            # If the heap is empty or we've reached our target count, add this batch to result
            if not heap:
        ...
        if self.verbose:
            print(f"Merged {len(merged)//2} batch pairs into {len(result)} batches")
    ...
    def _process_duplicate_batch(self, batch):
        ...
        for duplicate in tqdm(
            ...
            try:
                    ...
                    try:
                        ...
                        if file_path not in file_cache:
                            with open(file_path, "r", encoding="utf-8") as f:
                                file_cache[file_path] = json.load(f)
                ...
                if self.verbose:
                    print(
                        f"Error analyzing duplicate {pmid if 'pmid' in locals() else 'unknown'}: {str(e)}"
                    )
                results.append(
                    {
    ...
    def print_summary(self):
        ...
        print("=" * 60)
        print(f"Total duplicate sets analyzed: {summary['total_duplicate_sets']}")
        print(
        ...
        if self.results["partial_duplicates"]:
            ...
            for i, dup in enumerate(self.results["partial_duplicates"][:3]):
                print(f"\n{i+1}. PMID: {dup['pmid']}")
                print(
                    f"   Title: {dup['title'][:80]}{'...' if len(dup['title']) > 80 else ''}"
                )
                ...
                if dup["comparisons"]:
                    ...
                    print(
                        f"   Sentence match: {comp['identical_sentences'] + comp['similar_sentences']}/{min(comp['sentence_count1'], comp['sentence_count2'])} "
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Analyze sentence-level similarities between articles identified as duplicates."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=f"Similarity threshold for sentences to be considered matching (0.0-1.0, default: {DEFAULT_SIMILARITY_THRESHOLD})",
    )
    parser.add_argument(
        "--processes",
        type=int,
        default=min(cpu_count(), DEFAULT_MAX_CPU_LIMIT),
        help=f"Number of parallel processes to use (default: {min(cpu_count(), DEFAULT_MAX_CPU_LIMIT)})",
    )
    parser.add_argument(
        "--output",
        help="Output path for the analysis report (default: duplicate_sentence_analysis_<timestamp>.json)",
    )
    ...
    if args.sample > 0:
        ...
        if args.sample < original_count:
            ...
            analyzer.duplicates = random.sample(analyzer.duplicates, args.sample)

        print(
            f"Sampling mode: analyzing {len(analyzer.duplicates)} out of {original_count} duplicates ({len(analyzer.duplicates)/original_count*100:.2f}%)"
        )
