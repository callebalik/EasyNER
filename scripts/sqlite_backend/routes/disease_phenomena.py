...
def register_disease_phenomena_routes(
    app, get_db_simple_connection, visualization_manager: VisualizationManager
):
                ...
                def generate_sankey_visualization(_):
                    ...
                    try:
                        ...
                        cooccurrences = EntityCooccurrence.get_cooccurrences(
                            cursor=cursor, filters=filter_dict, limit=limit, offset=0
                        )
                        ...
                        logger.error(
                            f"Error in thread {thread_id}: {str(e)}", exc_info=True
                        )
                        return f"""
