        ...
        try:
            with open(full_config_path, "r") as f:
                ...
                if not config_data.get("develop") and not config_data.get("db_path"):
                    raise ValueError(
                        "Database configuration must contain 'db_path' unless in development mode."
                    )
                if not config_data.get("schema_path"):
                    raise ValueError(
                        "Database configuration must contain 'schema_path'."
                    )
                return config_data
        except (FileNotFoundError, json.JSONDecodeError, ValueError) as e:
            raise ValueError(
                f"Error loading database configuration from {config_path}: {e}"
            )
