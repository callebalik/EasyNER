...
class CooccurenceSankey:
    ...
    def create_sankey_diagram(self, cooccurrences: list[Cooccurrence]) -> go.Figure:
        ...
        try:
            ...
            self.logger.error(
                f"Error generating Sankey diagram: {str(e)}",
                exc_info=True,
            )
            return f"""
    ...
    def export_as_html(self, cooccurrences: list[Cooccurrence], filename: str) -> None:
        ...
        try:
            ...
            self.logger.error(
                f"Error exporting Sankey diagram: {str(e)}",
                exc_info=True,
            )
    ...
    def export_as_image(
        ...
        filename: str = "sankey_diagram.png",
    ) -> None:
    ...
    def _create_node_colors(self, processed_data: dict):
        ...
        def _generate_colors(
            ...
            colors = []
            max_count = max(
                (data[coloring_parameter] for data in nodes.values()),
                default=1,
            )
    ...
    def _create_link_data(self, processed_data):
        ...
        max_fq_doc_level = max(
            (getattr(cooc, "fq_doc_level", 1) or 1 for cooc in cooccurrences),
            default=1,
        )
        max_uniq_docs = max(
            (getattr(cooc, "uniq_docs", 1) or 1 for cooc in cooccurrences),
            default=1,
        )
    ...
    def _calculate_node_thicknesses(self, processed_data):
        ...
        max_disease_fq = max(
            (data[FQ_DOCUMENT_LEVEL] for data in e1_data.values()),
            default=1,
        )
        max_phenomenon_fq = max(
            (data[FQ_DOCUMENT_LEVEL] for data in e2_data.values()),
            default=1,
        )
