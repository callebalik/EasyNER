...
class DBDataExchanger:
    ...
    def _setup_necessary_views(self):
        ...
        VIEW_NE_CLEAN.refresh(cursor=self.cursor, logger=self.logger)
    ...
    def bulk_insert(self, insert_query: str, data: list[tuple]) -> None:
    ...
    def get_entity_occurrence(self, eo_id):
        ...
        try:
            self.cursor.execute(
                "SELECT * FROM entity_occurrences WHERE id = ?",
                (eo_id,),
            )
    ...
    def get_cooccurrences_summary(
        ...
        try:
            ...
            summaries = [dict(zip(columns, row, strict=False)) for row in rows]
    ...
    def rename_named_entity(self, old_name: str, new_name: str) -> None:
        try:
            self.cursor.execute(
