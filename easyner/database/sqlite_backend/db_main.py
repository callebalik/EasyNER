...
class EasyNerDBHandler:
    ...
    def _init_cache_minimal(self):
        ...
        try:
            ...
            from easyner.database.sqlite_backend.core.cache_singleton import (
                set_cache_manager_connection,
            )
    ...
    def _get_cached_value(self, key):
        ...
        try:
            row = self._connection.execute(
                "SELECT value, expires FROM _cache WHERE key = ?",
                (key,),
            ).fetchone()
    ...
    def reset_db_performance_settings(self) -> None:
    ...
    def executescript(self, sql_script) -> None:
    ...
    def align_with_schema(self, schema_file_path) -> None:
    ...
    def refresh_tables_info(self) -> None:
    ...
    def _initialize_components(self):
        ...
        try:
            ...
            from easyner.database.sqlite_backend.data_model.entity_occurrence import (
                EntityOccurrence,
            )
    ...
    def _setup_logging(self):
        ...
        if not os.path.exists(log_dir):
            msg = f"Failed to create log directory: {log_dir}"
            raise OSError(msg)
        ...
        if not os.access(log_dir, os.W_OK):
            ...
            msg = f"No write permission for log directory: {log_dir}"
    ...
    def execute_with_log(self, query, args=None) -> None:
