...
try:
    from easyner.utils.logging.table_log_formatter import TableLogFormatter
except ImportError:
...
class OperationMonitor:
    ...
    def __init__(self, logger):
        ...
        self._operation_counts = {"total": 0, "success": 0, "error": 0}
        # Configure alert thresholds from environment or defaults
        self.thresholds = {
            "memory_percent": float(
                os.environ.get("EASYNER_ALERT_THRESHOLD_MEMORY", 80.0),
            ),
            ...
            "slow_query_ms": float(
                os.environ.get("EASYNER_ALERT_THRESHOLD_SLOW_QUERY", 1000.0),
            ),
        }
    ...
    def _check_alerts(self, stats: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        if self._operation_counts["total"] > 0:
            error_rate = (
                self._operation_counts["error"] / self._operation_counts["total"]
            ) * 100
        ...
        if (
            "duration_ms" in stats
            and stats["duration_ms"] > self.thresholds["slow_query_ms"]
        ):
            alerts.append(
                {
                    "type": "SLOW_QUERY",
        ...
        try:
            ...
            for key, end_value in end_memory.items():
                if (
                    ...
                    and isinstance(start_memory[key], (int, float))
                ):
            ...
            if hasattr(self._local, "operations"):
                ops = [
                    op for op in self._local.operations if op["id"] != transaction_id
                ]
    ...
    def monitor_query(
        ...
        query_context = {"query": query, "params": params, **context}
    ...
    def register_periodic_monitor(self, app, interval: int = 60) -> None:
...
class DBConnectionMonitor:
    ...
    def register_connection(
        ...
        with self.lock:
            ...
            self.logger.debug(
                f"Registered connection {conn_id} from thread {thread_id}",
            )

    def unregister_connection(self, conn: sqlite3.Connection) -> None:
        ...
        with self.lock:
            for conn_id, info in list(self.connections.items()):
                try:
                    info["conn"].close()
                    self.logger.info(
                        f"Closed connection {conn_id} during emergency shutdown",
                    )
                except Exception as e:
        ...
        with self.lock:
            ...
            try:
                ...
                conn_info = {
                    ...
                    - self.connections[conn_id]["created_at"],
...
class ThreadMonitor:
    ...
    def register_thread(self, name: str, context: dict[str, Any] = None) -> None:
                ...
                result.append(
                    {
                        ...
                        "seconds_since_heartbeat": current_time
                        - info["last_heartbeat"],
                        "status": info["status"],
