...
class DataAnalyzer(BaseExecutor):
    ...
    def analyze_user_activity(self, user_ids):
        ...
        self.logger.info(
            f"Starting user activity analysis for {len(user_ids)} users in parallel...",
        )
        ...
        def _analyze_single_user(db_manager, logger, user_id):
            ...
            try:
                ...
                logger.debug(
                    f"Analysis for user {user_id} completed: {analysis_result}",
                )
        ...
        tasks = [
            lambda dbm, log, uid=uid: _analyze_single_user(dbm, log, uid)
            for uid in user_ids
        ]  # Create task lambdas
        ...
        self.logger.info(
            f"User activity analysis completed. Analyzed users: {len(processed_results)} out of {len(user_ids)} requested.",
        )
    ...
    def get_average_data_value(self, table_name, column_name):
        ...
        self.logger.info(
            f"Average value of '{column_name}' in '{table_name}': {average_value}",
        )
