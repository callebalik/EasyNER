...
if not os.path.exists(DBPATH):
    msg = f"Database path {DBPATH} does not exist"
    raise ValueError(msg)
if not os.path.isabs(DBPATH):
    msg = f"Database path {DBPATH} is not an absolute path"
    raise ValueError(msg)
...
def close_simple_connection(exception) -> None:
...
def align_with_schema():
    ...
    try:
        with get_db_easyner_context_connection() as db:
            ...
            with open(os.path.join(os.path.dirname(__file__), "indexes.sql")) as f:
                indexes_sql = f.read()
        ...
        with operation_monitor.monitor_operation("list_documents"):
            with get_db_easyner_context_connection() as db:
                ...
                return render_template(
                    "documents.html",
                    documents=[
                        dict(zip(["id", "title", "word_count"], doc, strict=False))
...
def list_entity_occurrences():
    ...
    return render_template(
        "table_view.html",
        table_name=f"{entities.VIEW_NE_COMP}",
        **result,
    )
...
def get_view_sample(view_name):
    try:
        ...
        view_check = cursor.execute(
            "SELECT type FROM sqlite_master WHERE type='view' AND name=?",
            [view_name],
        )
        if not view_check:
...
def show_views():
    try:
        ...
        return (
            render_template(
                "error.html",
                message=f"Error loading views page: {str(e)}",
            ),
            500,
        )
...
def list_aggregated_entity_occurrences():
    ...
    return render_template(
        "table_view.html",
        table_name="view_entity_occurrences_summary",
        **result,
    )
...
def dis_pnm_presentation():
    ...
    try:
        ...
        return render_template(
            "table_view.html",
            table_name=VIEW_DIS_PNM_CO_AGGR_ROW_FACTORY,
            **result,
        )
    except Exception as e:
...
if __name__ == "__main__":
    ...
    if not port_in_use:
        ...
        print("   $ kill <PID>         # Stop the process")
        print("2. Or specify a different port:")
        print("   $ python db_server.py --port 5002")
        print("3. Or set environment variable:")
        print("   $ export EASYNER_SERVER_PORT=5002 && python db_server.py")
