...
class Docs:
    ...
    def doc_calc_word_counts(self, batch_size=100000) -> None:
        ...
        """
        Calculate word count, token count, and alphabetic character count for each document in batches.
        Updates documents where counts are null or zero.
        """
        self.logger.info("Starting document counts calculation")
        start_time = time.time()
    ...
    def export_documents_to_json(self, doc_ids, output_path, metadata):
        ...
        try:
            ...
            self.logger.error(f"Failed to export documents to JSON: {e}")
            return None

    def export_zero_counts(self, output_path=None):
    ...
    def identify_with_zero_counts_export(self, output_path=None):
        """Identify and export documents with null or zero word/token counts to a JSON file.
        Prompts user for confirmation and output path if not provided.
        """
        self.cursor.execute(
            """
